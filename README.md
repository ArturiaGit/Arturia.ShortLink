...
[快速开始](#-快速开始-quickstart) • [核心特性](#-核心特性矩阵) • [系统架构](#-系统架构与技术拓扑) • [里程碑进度](#-研发里程碑与双线进度看板) • [AI 协同规范](#-ai-双-agent-协同工程范式) • [文档中心](#-规范文档索引库-docs)
...
- **工程范式**：独创 **AI 双 Agent 协同开发体系**（前端 Agent + 后端 Agent + 人机交付验收门禁），以严谨的契约冻结和规范文档驱动高质量敏捷迭代。
...
| **Phase 3** | 短链管理业务、Base62 发号与域名路由 | `[ ]` 待执行 | 规划 Base62 算法、密码加锁验证、软删除/封禁、独立域名 DnsClient 解析与二维码生成 |
| **Phase 4** | 高性能 302 重定向引擎与 Channels 削峰 | `[ ]` 待执行 | 规划毫秒级重定向端点、System.Threading.Channels 异步削峰、滑动窗口 UV 去重与批量回写 |
| **Phase 5** | UTM 分析统计、多级安全风控与部署镜像 | `[ ]` 待执行 | 规划分析时序/维度统计 API、API Key 双模鉴权、三层风控中间件、Docker 生产镜像与一键编排 |

---

## 🤖 AI 双 Agent 协同工程范式

本项目是 **AI-Native 软件工程 (AI-Native Software Engineering)** 的典型实践范例。仓库规范中定义了一套完整的人机协作与双 Agent 分工体系：
...
├───────────────────────┤                             ├───────────────────────┤
│ • 专职 frontend/ 目录  │                             │ • 专职 backend/ 目录  │
│ • Mock 引擎独立闭环   │                             │ • .NET 10 + MySQL 8   │
│ • 完成大阶段提审验收   │                             │ • 严格受限于契约门禁  │
└───────────┬───────────┘                             └───────────┬───────────┘
            │                                                     │
            │ 完成阶段验收                                         │ 100% 测试通过
...
├─────────────────────────────────────────────────────────────────────────────┤
│ • 未获用户明确批准前，严禁 commit / push                                     │
│ • 批准后特性分支自动提交、推送、发起 PR、Squash-merge 并同步主干             │
└─────────────────────────────────────────────────────────────────────────────┘
```

- **行动宪章**：请参阅 [`AGENTS.md`](AGENTS.md) 了解完整的 Agent 行动宪章、开发纪律与不可协商铁律。
- **契约冻结**：后端在进入功能编码前必须经由需求与契约冻结审查，确保前后端接口契约 100% 严密对齐。
...
| 规范文档 | 文档定位与核心内容 |
| :--- | :--- |
| 📑 [项目全局硬性约束.md](docs/项目全局硬性约束.md) | **【最高宪法】** 技术栈锁定、双 Agent 权责边界与大阶段质量门禁红线。 |
