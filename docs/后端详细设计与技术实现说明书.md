...
中间件拦截需要租户数据的 `/api/v1/*` 管理端请求。登录、注册、`/auth/me`、工作空间列表/创建/Slug 检查、健康检查、公网跳转与密码解锁不依赖当前租户；其中注册邀请消费、`/auth/me` 和工作空间列表通过受控内部查询读取跨空间邀请或当前用户的全部成员关系：
...
* **成功响应**：
  * 使用独立安全配置密钥生成绑定规范化 Host、Slug 与到期时间的 HMAC-SHA256 Base64Url 票据，验签采用固定时间比较；不得复用 JWT 或访客哈希密钥。
...
2. **当日 UV 内存去重算法**：
   ...
   * 服务启动时从当日 `link_access_log` 的去重哈希重建集合，跨重启保持单实例 UV 口径；每日清理过期条目。多实例精确去重属于第二期持久化消息与共享去重演进，不在 MVP 引入 Redis。
