...
| **v1.4.0** | 2026-09-16 | AI 代理 (Antigravity) | 确立三 Agent 协同角色流转标准，定义资产双轨 PR 隔离规范（全局规范优先独立 PR / 阶段资产特性分支统合 PR） |
...
1. **前置基线同步与特性分支切出 (Pre-flight Remote Sync & Branching)**：
   * **阶段启动前检查**：在开始执行任何一个大阶段（Phase）的开发任务前，Agent 必须运行 `git status --porcelain` 检查工作区状态。
   * **工作区约束与设计文档放行**：若检测到本地存在无关的脏代码改动，严禁私自切换分支或强行覆盖，必须停手向人类开发者告警；**但对于针对本大阶段刚刚输出并已暂存的《阶段详细设计说明书.md》（`docs/阶段详细设计/`）及清单初版标记，属于合法的阶段前置产物，允许 Agent 在切出特性分支时直接继承带入**。大阶段设计说明书属于该大阶段交付物的一部分，无需在编码前单独提 PR，统一随特性分支在最终的阶段 PR 中一同交付与合并。
   * **主干同步与分支切出**：确认工作区就绪后，切回 `main` 分支（或在暂存合法设计文档的状态下）切出独立的阶段特性分支，**前后端统一采用 `feat/phase-X-<name>` 格式**（例如后端阶段三切出 `feat/phase-3-shortlink-engine-api-keys`）。
...
5. **批准后清单勾选与最终复验**：
   * 用户明确同意后，Agent 必须仍在特性分支中将本阶段已验收项勾选为 `[x]`，**同步更新根目录 `AGENTS.md` 文首的「当前进度」摘要**，同步追加清单文档修订记录，然后重跑对应构建、测试、格式、Diff 与密钥扫描门禁。
...
### 5.2 资产双轨 PR 隔离规范 (Dual-Track PR Lifecycle)

在三 Agent 协同架构（前端 Agent、后端阶段详细设计 Agent、后端开发 Agent）运作中，针对代码库资产的生命周期确立严格的**双轨 PR 隔离与时序标准**：

```text
               ┌───► 阶段专属资产 ───► 保留在本地工作区 / 特性分支 ───► 阶段验收通过后统一 PR
代码库资产变更 ┤
               └───► 全局规范资产 ───► 立即解耦切出 docs/<name> 分支 ───► 优先发起独立 PR 合入 main
```

1. **阶段专属资产 (Phase-Specific Assets - 特性分支统合交付)**：
   * **资产范畴**：
     * 《阶段详细设计说明书.md》（位于 `docs/阶段详细设计/`）；
     * `backend/` 下的业务代码、实体、DTO、配置、数据库迁移与集成测试；
     * 《后端功能开发清单.md》中该阶段任务的 `[x]` 勾选与阶段完成修订记录；
     * 业务开发过程中针对本阶段对应端点微调的《接口契约规范.md》。
   * **生命周期与合并时机**：
     * 设计阶段完成后，说明书保留在本地工作区（或暂存区）；
     * 后端开发 Agent 开工时切出 `feat/phase-X-<name>` 分支直接继承带入；
     * 开发、自测、人工审阅通过后，**在同一个阶段特性分支中将上述资产一次性 Commit 并发起 PR**，Squash 合并入 `main`。
2. **全局 / 跨阶段规范资产 (Global Assets - 优先独立 PR 交付)**：
   * **资产范畴**：
     * 行为准则与分工宪章：`AGENTS.md`；
     * 项目全局硬性约束：`docs/项目全局硬性约束.md`；
     * 工作流与版本规范：`docs/Git工作流与Commit规范.md`、`docs/版本管理规范.md`；
     * 根目录通用说明：`README.md`、`LICENSE` 等。
   * **生命周期与合并时机**：
     * 一旦在阶段准备、设计讨论或架构重构中产生对全局规范的新增、修正或补充，**必须立即与业务阶段资产解耦**；
     * 排除阶段专属文件后，单独切出 `docs/<name>` 独立文档分支，提交并调用 `gh pr create` 发起 Pull Request；
     * 经过用户审阅或命令批准后，**优先 Squash 合并入 `main` 主干**；
     * 确保 `main` 主干上的全局规范始终处于最新且最高权威状态，严禁将全局宪法改动混杂在业务特性分支中延误生效。
