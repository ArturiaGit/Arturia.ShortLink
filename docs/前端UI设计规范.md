...
| 版本 | 修订日期 | 修订人 | 修订说明 |
| :--- | :--- | :--- | :--- |
| **v1.0.0** | 2026-09-06 | UI/UX 与前端团队 | 初始化前端 UI 设计体系规范（New York 预设、Zinc 调色板、组件分层架构、B1~B8 约束） |
| **v1.1.0** | 2026-09-07 | AI 代理 (Antigravity) | 新增 6.4 节短链卡片创建人标识视觉规范及 Member 无权操作时的置灰禁用交互规范 |
...
### 6.4 短链卡片元数据排版与细粒度 RBAC 权限态

1. **元数据行排版规范**：
   * 短链卡片底部信息行采用标准横向流式排版（`flex items-center gap-4 text-xs text-muted-foreground flex-wrap`）。
   * 必须依次包含：备注描述（单行截断）、**创建者身份胶囊标签**（`[ 👤 创建人姓名 ]`，采用 `bg-muted/40 border border-border/70 text-muted-foreground px-1.5 py-0.5 rounded-md text-[11px]`）、失效倒计时（若有到期时间）与创建日期。
2. **细粒度 RBAC 权限交互态**：
   * 当当前登录用户在工作空间内为 **Member (业务协作者)** 且该短链非本人所创建时：
     * **启停 Switch**：置灰禁用 (`disabled={!canManage}`)，鼠标悬浮 Tooltip 提示“业务协作者仅可启停本人创建的短链”；
     * **更多操作菜单**：
       * “编辑短链属性”项：置灰禁用，右侧展示淡灰色 `无权限` 字样；
       * “删除此短链”项：置灰禁用，右侧展示淡灰色 `无权限` 字样；
       * “定制专属二维码”与“访问目标长链接”保持对所有人开放可用。
