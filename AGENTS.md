...
* **协同分工模式 (三 Agent 协同架构)**：
  * **前端 Agent（独立前端会话）**：专职负责前端工程（React 19 + TypeScript + Vite + Tailwind CSS + shadcn/ui + Lucide Icons + Recharts），由高保真 Mock 引擎驱动独立闭环运行；**严禁编写任何 C# 后端代码**；每次完成前端大阶段验收时，**必须同步更新后端规范文档**。
  * **后端阶段详细设计 Agent（独立设计会话）**：专职负责各大阶段实施前的深度架构对齐与详细技术设计。通过 `/grill-me` 裁决技术决策，输出《后端阶段X详细设计说明书.md》（含落地技术方案），并在说明书附录与会话中输出针对后端开发 Agent 的**高精度结构化执行 Prompt**。
  * **后端开发 Agent（独立编码会话）**：专职负责后端工程代码实现与测试落地（.NET 10 + C# ASP.NET Core WebAPI + MySQL 8.0.46）。必须**先研读宏观阶段文档与契约，再深入研读本阶段具体实施方案**，完全理解后方可切分支开工；通过自动化测试与 Scalar 交互审阅闭环交付。
* **当前进度**：**前端阶段一至阶段六已 100% 达成验收，阶段七、八待执行；后端阶段一（脚手架与EF Core底座）、阶段二（用户认证、租户中间件与工作空间RBAC）、阶段三（短链核心映射、自定义别名、安全风控与双模 API Key）及阶段四（公网极速 302 重定向与 Channels 进程内削峰消费）已 100% 达成验收并合入 main；后端阶段五准备启动。**
...
│   │   └── 后端阶段三详细设计说明书.md # 【后端阶段三实操指南】Base62安全发号、自环风控缓存、SlidingWindow限流与双模API Key
...
1. **专职边界与三 Agent 协同架构**：
   * 前端 Agent 100% 仅在 `frontend/` 目录下工作，**绝对严禁编写任何 C# 后端代码、严禁创建后端工程目录**；后端工作由专职设计与开发 Agent 独立交付。
   * **跨 Agent 阶段设计与开发流转契约**：后端阶段详细设计 Agent 产出的设计说明书是开发 Agent 的唯一落地蓝图；后端开发 Agent 开工前必须先研读宏观基线（《后端详细设计与技术实现说明书.md》、《后端功能开发清单.md》、《接口契约规范.md》），再深入研读本阶段具体实施方案，彻底理解后方可切出分支开工。
   * **资产双轨 PR 隔离规则 (Dual-Track PR Lifecycle Rule)**：
     * **阶段专属资产**（阶段详细设计说明书、后端源码、测试、阶段清单勾选及开发中微调的本阶段对应接口契约）：保留在本地工作区与阶段特性分支中，无需提前提 PR，随该阶段全部代码自测与验收通过后，在同一个阶段 PR 中统一合入 `main`。
     * **全局 / 跨阶段规范资产**（`AGENTS.md`、`README.md`、`LICENSE`、`Git工作流与Commit规范.md`、`项目全局硬性约束.md` 等非阶段绑定的全局规范）：一旦在阶段准备或评审中发生新增或修改，**必须先单独切出 `docs/<name>` 分支提交 PR 提前合入 main**，保证主干宪法始终最新，严禁滞留在业务特性分支中延误生效。
...
5. **人机协同大阶段审阅、PR 与自动合并闭环（核心执行铁律）**：
   ...
   * **阶段启动前切出特性分支**：检查工作区状态；若存在针对本阶段的详细设计说明书（`docs/阶段详细设计/`）或清单初版暂存修改，允许直接带入特性分支；确保 main 基准对齐后切出阶段独立分支 `feat/phase-X-<name>` 开发。
   * **阶段详细设计说明书交付规范**：各大阶段对应的《阶段详细设计说明书.md》属于该大阶段交付物的一部分，无需在编码前单独提前提 PR，允许作为该阶段初始成果随 `feat/phase-X-<name>` 特性分支一同推进，开发自测通过后在同一个阶段 PR 中统一合入。
   ...
   * 用户确认同意后，Agent 先在特性分支勾选本阶段清单、**同步更新根目录 `AGENTS.md` 文首的「当前进度」摘要**、更新修订记录并复跑门禁，再执行 Conventional Commit、推送、`gh pr create` 与 `gh pr merge --squash --delete-branch`；切回 `main` 同步并验证清单与 `AGENTS.md` 已合入、工作区纯净，随后**必须立即原地暂停**。
6. **语言与命名标准**：
...
```text
步骤 0: 基线同步与分支切出 ──► 检查工作区纯净(或带入阶段设计暂存)，main pull --rebase，切出 feat/phase-X-<name>
    │
    ▼
...
步骤 5: 人工确认 ──► 停下操作，汇报本阶段完成成果，等待用户确认
    │   ├── 若需修改 ──► 就地修改后重新打开浏览器审阅
    │   └── 用户同意 ──► 在特性分支勾选清单、同步刷新 AGENTS.md 当前进度、更新修订记录并复跑全部门禁
    ▼
